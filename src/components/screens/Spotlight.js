// import React, { Component } from "react";
import { Link } from "react-router-dom";
import styled from "styled-components";
import Logo from "../.././Assets/images/Logo-Food-Site.svg";
import Dollars from "../../Assets/images/dollar.svg";
import products from "../json/Product.json";
import { useState } from "react";
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";

function Spotlight() {
    const [startDate, setStartDate] = useState(new Date());
    const [orders, setOrders] = useState([]);
    // add to cart
    const addToCart = (id) => {
        let new_food = products.find((products) => products.id === id);
        setOrders([new_food, ...orders]);
    };
    // render order function
    const renderOrder = () => {
        return orders.map((product) => {
            return (
                <BreakFastList>
                    <BreakFastLeft>
                        <BreakFastImgCont>
                            <BreakFastImg
                                src={require(`../../Assets/images/${product.image}`)}
                            ></BreakFastImg>
                        </BreakFastImgCont>
                    </BreakFastLeft>
                    <BreakFastRight>
                        <Name>{product.title} </Name>
                        <Pricing>
                            <DisCounter>
                                <Discount>
                                    <DollarSign>$</DollarSign>
                                    {product.Discount_Price}
                                </Discount>
                            </DisCounter>
                            <Actual>{product.Actual_Price}</Actual>
                        </Pricing>
                    </BreakFastRight>
                </BreakFastList>
            );
        });
    };
    // function to render date
    function renderDate() {
        return (
            <DatePicker
                selected={startDate}
                onChange={(date) => setStartDate(date)}
                dateFormat="dd MMM yyyy"
            />
        );
    }
    // rendering products
    let renderProducts = () => {
        return products.map((product) => {
            return (
                <FoodCard>
                    <FoodImageBox>
                        <FoodImage
                            src={require(`../../Assets/images/${product.image}`)}
                        />
                    </FoodImageBox>
                    <FoodDetailsBox>
                        <CardTop>
                            <Para>{product.title}</Para>
                        </CardTop>
                        <CardBottom>
                            <PriceSection>
                                <PriceContainer>
                                    <DollarImg>
                                        <Dollar src={Dollars} />
                                    </DollarImg>
                                    <Price>{product.Discount_Price}</Price>
                                </PriceContainer>
                                <PriceOff>{product.Actual_Price}</PriceOff>
                            </PriceSection>
                            <Purchase onClick={() => addToCart(product.id)}>
                                Order
                            </Purchase>
                        </CardBottom>
                    </FoodDetailsBox>
                </FoodCard>
            );
        });
    };

    return (
        <Spotlights>
            <LeftContainer>
                <LogoBox>
                    <Logoimg src={Logo} />
                </LogoBox>
                <NavBox>
                    <ImageContainer>
                        <Image
                            src={
                                require("../../Assets/images/Menu-White.svg")
                                    .default
                            }
                        />
                    </ImageContainer>
                    <ImageContainer>
                        <Image
                            src={
                                require("../../Assets/images/circlepiece.svg")
                                    .default
                            }
                        />
                    </ImageContainer>
                    <ImageContainer>
                        <Image
                            src={
                                require("../../Assets/images/Love-white.svg")
                                    .default
                            }
                        />
                    </ImageContainer>
                    <ImageContainer>
                        <Image
                            src={
                                require("../../Assets/images/square.svg")
                                    .default
                            }
                        />
                    </ImageContainer>
                    <ImageContainer>
                        <Image />
                    </ImageContainer>
                    <ImageContainer>
                        <Image
                            src={
                                require("../../Assets/images/cart.svg").default
                            }
                        />
                    </ImageContainer>
                    <ImageContainer>
                        <Image
                            src={
                                require("../../Assets/images/settings.svg")
                                    .default
                            }
                        />
                    </ImageContainer>
                    <ImageContainer>
                        <Image
                            src={
                                require("../../Assets/images/profile.svg")
                                    .default
                            }
                        />
                    </ImageContainer>
                    <ImageContainer>
                        <Image
                            src={
                                require("../../Assets/images/exclamation.svg")
                                    .default
                            }
                        />
                    </ImageContainer>
                </NavBox>
                <AvatarBox>
                    <Avatar
                        src={require("../../Assets/images/profile.svg").default}
                    />
                </AvatarBox>
            </LeftContainer>
            {/* // LeftContainer section completed */}
            <MiddleContainer>
                <Header>
                    <Content>
                        <Heading>Welcome, Epan Danis</Heading>
                        <Paragraph>
                            Don't forget to have breakfast today yeahhh!
                        </Paragraph>
                    </Content>
                    <SideMenu>
                        <Button1>Menu</Button1>
                        <Button2>Meal plans</Button2>
                        <Button3>Delivery</Button3>
                        <Form>
                            <SearchBar placeholder="Search your food" />
                        </Form>
                    </SideMenu>
                </Header>
                <MainSection>
                    <SubHeading>Find the menu you want</SubHeading>
                    <Items>
                        <ItemsList>
                            <RecommendedButton to="/">
                                Recommended
                            </RecommendedButton>

                            <BreakfastButton to="/breakfast">
                                Breakfast
                            </BreakfastButton>

                            <LunchButton to="/lunch">Lunch</LunchButton>

                            <DinnerButton to="/dinner">Dinner</DinnerButton>

                            <IceCreamButton to="/icecream">
                                IceCream
                            </IceCreamButton>

                            <CoffeeButton to="/coffee">Coffee</CoffeeButton>
                        </ItemsList>
                    </Items>
                </MainSection>
                <MainMenu>{renderProducts()}</MainMenu>
            </MiddleContainer>
            {/* Middle container completed */}
            {/* Structuring of Right container starts */}
            <RightContainer>
                <DateContainer>
                    <HeadLine>Your meal plan</HeadLine>
                    <DateBar>
                        {renderDate()}
                        <CalenderContainer>
                            <Calender
                                src={
                                    require("../../Assets/images/calender.svg")
                                        .default
                                }
                            ></Calender>
                        </CalenderContainer>
                        <ArrowContainer>
                            <Arrow
                                src={
                                    require("../../Assets/images/arrowdown.svg")
                                        .default
                                }
                            ></Arrow>
                        </ArrowContainer>
                    </DateBar>
                </DateContainer>

                <BreakfastContainer>
                    <CaptionContainer>
                        <Banner>Breakfast</Banner>
                        <Purchased>Purchased</Purchased>
                    </CaptionContainer>
                    <Time>06.30AM</Time>
                    <BreakFast>{renderOrder()}</BreakFast>
                </BreakfastContainer>

                <LunchContainer>
                    <CaptionContainer>
                        <Lemma>Lunch</Lemma>
                        <Being>Being ordered</Being>
                    </CaptionContainer>
                    <Time>01.30PM</Time>
                    <Lunch>
                        <LunchList>
                            <LunchLeft>
                                <LunchImgCont>
                                    <LunchImg
                                        src={require("../../Assets/images/one.jpg")}
                                    ></LunchImg>
                                </LunchImgCont>
                            </LunchLeft>
                            <LunchRight>
                                <Name>Nasi Goreng Park Parjo </Name>
                                <Pricing>
                                    <Discount>
                                        <DollarSign>$</DollarSign>8,10
                                    </Discount>
                                    <Actual>$10,10</Actual>
                                </Pricing>
                            </LunchRight>
                        </LunchList>
                    </Lunch>

                    <Lunch>
                        <LunchList>
                            <LunchLeft>
                                <LunchImgCont>
                                    <LunchImg
                                        src={require("../../Assets/images/one.jpg")}
                                    ></LunchImg>
                                </LunchImgCont>
                            </LunchLeft>
                            <LunchRight>
                                <Name>Nasi Goreng Park Parjo </Name>
                                <Pricing>
                                    <Discount>
                                        <DollarSign>$</DollarSign>8,10
                                    </Discount>
                                    <Actual>$10,10</Actual>
                                </Pricing>
                            </LunchRight>
                        </LunchList>
                    </Lunch>
                </LunchContainer>

                <DinnerContainer>
                    <CaptionContainer>
                        <Banner>Dinner</Banner>
                        <SmallSpan>Haven't ordered yet</SmallSpan>
                    </CaptionContainer>
                    <Time>09.30PM</Time>
                    <Dinner>
                        <DinnerList>
                            <DinnerLeft>
                                <DinnerImgCont>
                                    <DinnerImg
                                        src={require("../../Assets/images/night.jpg")}
                                    ></DinnerImg>
                                </DinnerImgCont>
                            </DinnerLeft>
                            <DinnerRight>
                                <Name>Nasi Goreng Park Parjo </Name>
                                <Pricing>
                                    <Discount>
                                        <DollarSign>$</DollarSign>8,10
                                    </Discount>
                                    <Actual>$10,10</Actual>
                                </Pricing>
                            </DinnerRight>
                        </DinnerList>
                    </Dinner>
                </DinnerContainer>
                <DropContainer>
                    <Drop>Drop here to add</Drop>
                </DropContainer>
            </RightContainer>
            {/* Structuring of right container ends */}
        </Spotlights>
    );
}
const Spotlights = styled.section`
    height: 100vh;
    display: flex;
`;
const LogoBox = styled.div`
    width: 56px;
    margin-bottom: 8.5rem;
`;
const Logoimg = styled.img`
    display: block;
    width: 100%;
`;
const LeftContainer = styled.section`
    width: 8%;
    padding: 10px 20px;
`;
const NavBox = styled.ul`
    list-style: none;
    background: #f46801;
    padding: 20px 12px;
    border-radius: 25px;
    width: 3rem;
`;
const ImageContainer = styled.li`
    width: 18px;
    padding-bottom: 1rem;
    margin: 1px auto 5px auto;
    &:hover {
        cursor: pointer;
    }
`;
const Image = styled.img`
    display: block;
    width: 100%;
`;
const AvatarBox = styled.div`
    width: 50px;
    /* margin-top: 5rem;
    padding-bottom: 15px; */
`;
const Avatar = styled.img`
    display: block;
    width: 100%;
    border-radius: 50%;
`;
const Header = styled.section`
    display: flex;
    align-items: center;
    height: 100px;
`;
const Content = styled.div``;
const Heading = styled.h2`
    font-size: 23px;
    font-weight: 700;
`;
const Paragraph = styled.p`
    font-size: 12px;
    font-family: "Inter regular";
`;
const SideMenu = styled.div`
    display: flex;
    justify-content: space-around;
    align-items: center;
`;
const Button1 = styled.button`
    display: inline;

    border: none;
    padding: 15px 24px;
    border-radius: 25px;
    text-align: center;
    color: #747474;
    background: #e1e1e1;
    &:hover {
        background: #f46801;
    }
`;
const Button2 = styled.button`
    display: inline;
    border: none;
    border-radius: 25px;
    padding: 15px 24px;
    margin-left: 1rem;
    text-align: center;
    color: #747474;
    background: #e1e1e1;
    &:hover {
        background: #f46801;
    }
`;
const Button3 = styled.button`
    display: inline;
    border: none;
    border-radius: 25px;
    padding: 15px 24px;
    margin-left: 1rem;
    text-align: center;
    color: #747474;
    background: #e1e1e1;
    &:hover {
        background: #f46801;
    }
`;
const Form = styled.form`
    margin-left: 2rem;
`;
const SearchBar = styled.input`
    border: none;
    background-color: #e9e9e9;
    padding: 14px 25px;
    border-radius: 25px;
    font-family: "Inter regular";
`;
const MainSection = styled.section``;
const SubHeading = styled.h2`
    font-size: 23px;
    font-weight: 500;
    margin-bottom: 2rem;
`;
const Items = styled.div`
    margin-bottom: 3rem;
`;
const ItemsList = styled.div`
    list-style: none;
    display: flex;
    justify-content: space-between;
    border-bottom: 0.5px solid #747474;
`;
const RecommendedButton = styled(Link)`
    color: #747474;
    display: block;
    margin-bottom: 0.5rem;
`;
const BreakfastButton = styled(Link)`
    color: #747474;
    display: block;
`;
const LunchButton = styled(Link)`
    color: #747474;
    display: block;
`;
const DinnerButton = styled(Link)`
    color: #747474;
    display: block;
`;
const IceCreamButton = styled(Link)`
    color: #747474;
    display: block;
`;
const CoffeeButton = styled(Link)`
    color: #747474;
    display: block;

    margin-right: 20px;
`;
const MiddleContainer = styled.section`
    width: 70%;
`;
const MainMenu = styled.section``;
const FoodCard = styled.div`
    background-color: #ede7e7;
    margin-right: 2rem;
    margin-bottom: 2rem;
    display: inline-block;
    border-radius: 10px;
    width: 28%;
`;
const FoodImageBox = styled.div`
    width: 250px;
    margin-bottom: 2rem;
`;
const FoodImage = styled.img`
    border-radius: 10px;
    display: block;
    width: 100%;
`;
const FoodDetailsBox = styled.div``;
const CardTop = styled.div``;
const Para = styled.p`
    font-weight: 700;
    margin-left: 1rem;
    margin-bottom: 1rem;
    font-family: "Inter regular";
`;
const CardBottom = styled.div`
    display: flex;
    margin-bottom: 1rem;
    justify-content: space-around;
`;
const PriceSection = styled.div`
    display: flex;
`;
const PriceContainer = styled.div`
    display: flex;
    align-items: center;
`;
const DollarImg = styled.div`
    width: 15px;
`;
const Dollar = styled.img`
    display: block;
    width: 100%;
`;
const Price = styled.span`
    font-size: 25px;
`;
const PriceOff = styled.span`
    margin-left: 1rem;
    color: #747474;
    text-decoration: line-through;
`;
const Purchase = styled.button`
    margin-left: 4rem;
    background: #f46801;
    padding: 3px 10px;
    border-radius: 25px;
    color: #fff;
    border: none;
    &:hover {
        cursor: pointer;
    }
`;

const RightContainer = styled.aside`
    background: #eeeeee;
    width: 25%;
    padding: 0 15px;
    height: 810px;
    @media all and (max-width: 768px) {
        width: 25%;
        padding: 0 8px;
    }
    @media all and (max-width: 640px) {
        width: 30%;
        padding: 0 8px;
    }
`;
const DateContainer = styled.div`
    padding-top: 20px;
`;
const HeadLine = styled.h4`
    font-size: 18px;
    font-weight: 700;
    margin-bottom: 10px;
    @media all and (max-width: 980px) {
        font-size: 15px;
    }
    @media all and (max-width: 480px) {
        font-size: 11px;
    }
`;
const DateBar = styled.div`
    position: relative;
`;
const CalenderContainer = styled.div`
    width: 22px;
    top: 7px;
    position: absolute;
    left: 7px;
    @media all and (max-width: 980px) {
        width: 20px;
    }
    @media all and (max-width: 480px) {
        display: none;
    }
`;
const Calender = styled.img`
    display: block;
    width: 100%;
`;
const ArrowContainer = styled.div`
    width: 18px;
    position: absolute;
    top: 10px;
    right: 10px;
    @media all and (max-width: 980px) {
        width: 13px;
        right: 11px;
    }
    @media all and (max-width: 480px) {
        display: none;
    }
`;
const Arrow = styled.img`
    display: block;
    width: 100%;
`;
const BreakfastContainer = styled.div`
    // padding-bottom: 10px;
`;
const CaptionContainer = styled.div`
    display: flex;
    justify-content: space-between;
    margin-top: 20px;
    margin-bottom: 20px;
    @media all and (max-width: 480px) {
        margin-top: 10px;
        margin-bottom: 5px;
    }
`;
const SmallSpan = styled.span``;
const Banner = styled.h5`
    font-size: 15px;
    font-weight: 700;
    @media all and (max-width: 980px) {
        font-size: 12px;
    }
    @media all and (max-width: 640) {
        font-size: 11px;
    }
`;
const Purchased = styled.div`
    background: #d9ffe5;
    color: #5cbe7b;
    font-size: 11px;
    border-radius: 10px;
    padding: 3px 11px;
    font-weight: 700;
    @media all and (max-width: 980px) {
        font-size: 9px;
        // margin-left: 8px;
        padding: 3px 10px;
    }
    @media all and (max-width: 480px) {
        display: none;
    }
`;
const Time = styled.h5`
    font-size: 12px;
    font-weight: 700;
    margin-bottom: 10px;
    @media all and (max-width: 480px) {
        font-size: 10px;
    }
`;
const Name = styled.h5`
    font-size: 10px;
    font-weight: 700;
    margin-bottom: 10px;
    @media all and (max-width: 980px) {
        font-size: 9px;
    }
`;
const Pricing = styled.div`
    display: flex;
    justify-content: space-between;
    align-items: center;
    width: 60%;
`;
const DollarSign = styled.span`
    display: inline-block;
    font-size: 10px;
    color: var(--orange);
    @media all and (max-width: 980px) {
        display: inline;
    }
`;
const Discount = styled.h4`
    font-size: 12px;
    font-weight: 700;
    @media all and (max-width: 980px) {
        font-size: 10px;
    }
`;
const Actual = styled.h6`
    font-size: 10px;
    text-decoration: line-through;
    color: #747474;
    @media all and (max-width: 980px) {
        font-size: 9px;
        margin-left: 5px;
    }
`;
const LunchContainer = styled.div``;
const Lemma = styled.h5`
    font-size: 15px;
    font-weight: 700;
    @media all and (max-width: 980px) {
        font-size: 12px;
    }
`;

const Being = styled.div`
    font-size: 10px;
    border-radius: 10px;
    background: #fef3df;
    color: #dfb77f;
    padding: 3px 11px;
    font-weight: 700;
    @media all and (max-width: 980px) {
        margin-left: 7px;
        font-size: 9px;
        padding: 3px 10px;
    }
    @media all and (max-width: 640px) {
        padding: 3px 6px;
    }
    @media all and (max-width: 480px) {
        display: none;
    }
`;

const Lunch = styled.ul``;
const LunchList = styled.li`
    display: flex;
    justify-content: space-between;
    align-items: center;
    margin-bottom: 10px;
    background-color: #fff;
    border-radius: 6px;
`;
const LunchLeft = styled.div`
    width: 35%;
    @media all and (max-width: 980px) {
        width: 70%;
    }
    @media all and (max-width: 480px) {
        display: none;
    }
`;
const LunchImgCont = styled.div`
    width: 100%;
    overflow: hidden;
    border-bottom-left-radius: 6px;
    border-top-left-radius: 6px;
    @media all and (max-width: 980px) {
        width: 95%;
    }
`;
const LunchImg = styled.img`
    display: block;
    width: 100%;
`;
const LunchRight = styled.div`
    width: 62%;
    @media all and (max-width: 980px) {
        width: 70%;
    }
    @media all and (max-width: 480px) {
        width: 100%;
        margin-left: 3px;
    }
`;

const DinnerContainer = styled.div``;

const Dinner = styled.ul``;
const DinnerList = styled.li`
    display: flex;
    justify-content: space-between;
    align-items: center;
    margin-bottom: 10px;
    background-color: #fff;
    border-radius: 6px;
`;
const DinnerLeft = styled.div`
    width: 35%;
    @media all and (max-width: 480px) {
        display: none;
    }
`;
const DinnerImgCont = styled.div`
    width: 100%;
    overflow: hidden;
    border-bottom-left-radius: 6px;
    border-top-left-radius: 6px;
`;
const DinnerImg = styled.img`
    display: block;
    width: 100%;
`;
const DinnerRight = styled.div`
    width: 62%;
    @media all and (max-width: 480px) {
        width: 100%;
        margin-left: 3px;
    }
`;
const DropContainer = styled.div`
    padding-top: 10px;
`;
const Drop = styled.div`
    font-size: 16px;
    border-radius: 10px;
    border: 1px dashed #eac1a5;
    padding: 10px 50px;
    color: #d8772a;
    font-weight: 600;
    background-color: #ffecde;
    @media all and (max-width: 1080px) {
        padding: 10px 30px;
    }
    @media all and (max-width: 980px) {
        padding: 10px 15px;
        font-size: 11px;
    }
    @media all and (max-width: 480px) {
        display: none;
    }
`;
const BreakFast = styled.ul``;
const BreakFastList = styled.li`
    display: flex;
    justify-content: space-between;
    align-items: center;
    margin-bottom: 10px;
    background-color: #fff;
    border-radius: 6px;
`;
const BreakFastLeft = styled.div`
    width: 35%;
    @media all and (max-width: 980px) {
        width: 70%;
    }
    @media all and (max-width: 480px) {
        display: none;
    }
`;
const BreakFastImgCont = styled.div`
    width: 100%;
    overflow: hidden;
    border-bottom-left-radius: 6px;
    border-top-left-radius: 6px;
    @media all and (max-width: 980px) {
        width: 95%;
    }
`;
const DisCounter = styled.div``;
const BreakFastImg = styled.img`
    display: block;
    width: 100%;
`;
const BreakFastRight = styled.div`
    width: 62%;
    @media all and (max-width: 980px) {
        width: 70%;
    }
    @media all and (max-width: 480px) {
        width: 98%;
        margin-left: 3px;
    }
`;

export default Spotlight;
